...
def process_green(in_year, in_month, in_day):
    ...
    try:
        ...
        green_trip_data_dest_loc = data_dir + "/NYC_Trip_Data_Loc_To_Zone/green_tripdata/"
        if not os.path.exists(green_trip_data_dest_loc):
            os.mkdir(green_trip_data_dest_loc)
        green_trip_parquet_dir_name = "result_" + in_year + "_" + in_month + ".parquet"
        green_trip_parquet_dir_name = green_trip_data_dest_loc + green_trip_parquet_dir_name
        if not os.path.exists(green_trip_parquet_dir_name):
            os.mkdir(green_trip_parquet_dir_name)
        green_trip_date_dir = "/p_date=" + in_year + "-" + in_month + "-" + in_day
        green_trip_date_dir = green_trip_parquet_dir_name + green_trip_date_dir
        if not os.path.exists(green_trip_date_dir):
            os.mkdir(green_trip_date_dir)
        result.to_parquet(green_trip_date_dir + "/p_date=" + in_year + "-" + in_month + "-" + in_day + ".parquet")
        # Save log
        e_time = time.time()
        y_time_log = open(trip_data_dest_loc + "green_time_log.txt", "a+")
        ...
        y_error_log = open(trip_data_dest_loc + "green_error_log.txt", "a+")
        ...
        y_error_log = open(trip_data_dest_loc + "green_error_log.txt", "a+")
        y_error_log.writelines(
            "Exception: ATE " + in_year + " " + in_month + " " + in_day + "'s data not processed." + "\n")
        y_error_log.close()
        print("Exception: VTE " + in_year + " " + in_month + " " + in_day + "'s data not processed.")
        # exit(1)
...
if __name__ == '__main__':
    ...
    error_log = open(trip_data_dest_loc + "green_error_log.txt", "w+")
    time_log = open(trip_data_dest_loc + "green_time_log.txt", "w+")
    error_log.close()
    time_log.close()
    for i in range(0, len(date_list), 8):
        try:
            p1 = Process(target=process_green, args=(date_list[i][0], date_list[i][1], date_list[i][2],))
            p2 = Process(target=process_green, args=(date_list[i + 1][0], date_list[i + 1][1], date_list[i + 1][2],))
            p3 = Process(target=process_green, args=(date_list[i + 2][0], date_list[i + 2][1], date_list[i + 2][2],))
            p4 = Process(target=process_green, args=(date_list[i + 3][0], date_list[i + 3][1], date_list[i + 3][2],))
            p5 = Process(target=process_green, args=(date_list[i + 4][0], date_list[i + 4][1], date_list[i + 4][2],))
            p6 = Process(target=process_green, args=(date_list[i + 5][0], date_list[i + 5][1], date_list[i + 5][2],))
            p7 = Process(target=process_green, args=(date_list[i + 6][0], date_list[i + 6][1], date_list[i + 6][2],))
            p8 = Process(target=process_green, args=(date_list[i + 7][0], date_list[i + 7][1], date_list[i + 7][2],))
            p1.start()
            p2.start()
            p3.start()
            p4.start()
