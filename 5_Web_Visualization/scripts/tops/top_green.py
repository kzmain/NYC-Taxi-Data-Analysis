...
county_rates = []
count = 0
top_10 = list(year_df.nlargest(20, ['gcount'])['gcount'])
for index, row in soa_shape_map_geo.iterrows():
    ...
    if year_df.iloc[count]['gcount'] in top_10:
        county_rates.append(year_df.iloc[count]['gcount'])
    else:
...
p = figure(
    title="TOP 10 GREEN TAXI ZONES", tools=TOOLS,
